...
def run_test(test_data, case_name=""):
    """运行单组测试"""
    expected_avg = sum(test_data.values()) // len(test_data)
    print(f"\n{'='*55}")
    if case_name:
    ...
    # ── 验证 ─────────────────────────────────────────────
    status = "✓ 通过" if avg == expected_avg else "✗ 失败"
    print(f"\n  {status}！  平均值 = {avg}  (预期 {expected_avg})")
...
# 测试用例1：常规数据（10, 20, 30）
run_test({1: 10, 2: 20, 3: 30}, "常规数据")

# 测试用例2：含0的数据（0, 15, 30）
run_test({1: 0, 2: 15, 3: 30}, "含0数据")

# 测试用例3：较大数据（100, 200, 300）
run_test({1: 100, 2: 200, 3: 300}, "较大数据")
